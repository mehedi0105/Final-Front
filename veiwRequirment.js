window.onload = () => {
    ...
    // Clear the parent container if it has any content
    if (parent.innerHTML !== "") {
        parent.innerHTML = "";
    }
    ...
    fetch(`https://final-s1v0.onrender.com/seller/project_requirment/`)
        ...
        .then(async (data) => {
            ...
            // Use a for...of loop to handle async operations within the loop
            for (const element of data) {
                console.log(element.id, "  ", id);

                // Ensure only the matching job ID is processed once
                if (String(element.job) === id && cnt === 0) {
                    ...
                    // Create the job overview and insert it into the DOM
                            ...
                            <div class="card top-box mt-4" style="border: none">
                                <div class="card-body top-box-body d-flex">
                                    ...
                                    <div class="flex-grow-1">
                                        <h5>${company_name}</h5>
                                        <p class="text-muted">${company_name} IT Solution</p>
                                    ...
                                    <div>
                                      <button type="button" class="btn text-white job-details-btn" onclick="SaveSubmitRequirmentData(${element.job})" style="background-color: #26ae61; padding: 15px;" data-bs-toggle="modal" data-bs-target="#applyModal">Accept Job</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="card" style="background: #fdfdfd; border: none;">
                                        <div class="card-body">
                                            <h5 class="card-title">Job Requirement Overview</h5>
                                            <ul class="list-unstyled job-overview">
                                                ...
                                                <li><i class="fa-solid fa-envelope job-overview-icon"></i>Requirement: <br>
                                                <p style="margin-left: 46px;" class="Description">${element.requirment}</p>
                                                </li>
                                            </ul>
                                            <button type="button" class="btn text-white w-100" onclick="SaveSubmitRequirmentData(${element.job})" style="background-color: #26ae61; padding: 15px;" data-bs-toggle="modal" data-bs-target="#applyModal">Accept Job</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
